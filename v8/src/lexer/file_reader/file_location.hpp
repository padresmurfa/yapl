...
public:
    FileLocation();
    FileLocation(const std::string& filename, size_t lineNumber, size_t fileOffsetInBytes, size_t lineOffsetInBytes = 0);
    FileLocation(const FileLocation& other);
    FileLocation& operator=(const FileLocation& other);

    const std::string& getFilename() const;
    size_t getLineNumber() const;
    size_t getFileOffsetInBytes() const;
