...
void handlingParenthesis(const preprocessor::PreprocessorToken &token, DecontextualizerContext& context) {
    switch (token.type) {
        ...
        case preprocessor::PreprocessorTokenType::MINUS_MINUS_MINUS:
            throw context.invalidTokenInThisContextException(token, DecontextualizerState::HANDLING_PARENTHESIS, "A comment-line-separator token is not expected as a token within a parenthesis-block");
