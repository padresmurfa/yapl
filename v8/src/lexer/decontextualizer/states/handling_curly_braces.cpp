...
void handlingCurlyBraces(const preprocessor::PreprocessorToken &token, DecontextualizerContext& context) {
    switch (token.type) {
        ...
        case preprocessor::PreprocessorTokenType::SEMICOLON:
            throw context.invalidTokenInThisContextException(token, DecontextualizerState::HANDLING_CURLY_BRACES, "A semicolon token is not expected as a token within a curly-braces-block");
