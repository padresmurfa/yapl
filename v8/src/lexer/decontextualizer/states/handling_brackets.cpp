...
void handlingBrackets(const preprocessor::PreprocessorToken &token, DecontextualizerContext& context) {
    switch (token.type) {
        case preprocessor::PreprocessorTokenType::CLOSE_BRACKET:
            {
                context.pop(DecontextualizerState::HANDLING_BRACKETS, token);
            }
            break;

        case preprocessor::PreprocessorTokenType::ESCAPED_CHARACTER:
            throw context.invalidTokenInThisContextException(token, DecontextualizerState::HANDLING_BRACKETS, "An escaped character is not expected as a token within a brackets-block");

        case preprocessor::PreprocessorTokenType::COLON:
            throw context.invalidTokenInThisContextException(token, DecontextualizerState::HANDLING_BRACKETS, "A colon is not expected as a token within a brackets-block");
        ...
        case preprocessor::PreprocessorTokenType::MINUS_MINUS_MINUS:
            throw context.invalidTokenInThisContextException(token, DecontextualizerState::HANDLING_BRACKETS, "A comment-line-separator token is not expected as a token within a brackets-block");
