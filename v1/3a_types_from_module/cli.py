...
class Extractor:
    ...
    def get_filename(self, build):
        return str(Path(build) / "types" / self._metadata["source"]["revision"] / self._metadata["identifier"])
...
class ModuleTypeExtractor(Extractor):
    ...
    def __init__(self, parent, line):
        super().__init__(parent._metadata, "types")
        self._parent = parent
        m = ModuleTypeExtractor._expression.match(line)
        _private = m.group(1) is not None and m.group(1).startswith("private")
        _type = m.group(2)
        remainder = m.group(len(m.groups()))
        m = ModuleTypeExtractor._identifier.match(remainder)
        _name = m.group(1)
    ...
    def get_filename(self, build):
        _path = self.get_path(build)
        return str(Path(_path) / self._metadata["type"]["name"])
