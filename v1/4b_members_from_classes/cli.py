...
def main(parsed_args):
    ...
    class_members_path = Path(parsed_args.build) / "class_members"
    class_members_path.mkdir(parents=True, exist_ok=True)
    if parsed_args.force:
        shutil.rmtree(class_members_path, ignore_errors=False, onerror=None)
        class_members_path.mkdir(parents=True, exist_ok=True)
...
class Extractor:
    ...
    def get_filename(self, build):
        return str(Path(build) / "class_members" / self._metadata["source"]["revision"] / self._metadata["identifier"])
...
class ClassMemberVariableExtractor(Extractor):
    ...
    def __init__(self, parent, line):
        super().__init__(parent._metadata, "members")
        self._parent = parent
        m = ClassMemberVariableExtractor._expression.match(line)
    ...
    def get_path(self, build):
        _path = str(Path(build) / "class_members" / self._metadata["source"]["revision"] / self._metadata["class"]["name"])
        return str(_path)

    def get_filename(self, build):
        _path = self.get_path(build)
        return str(Path(_path) / self._metadata["member"]["name"])
        ...
        for line in self._lines:
            ...
            if extractor is not None:
                if not extractor.process_line(line):
                    extractor.save(build)
                    extractor = None
            elif ClassMemberVariableExtractor.matches(line):
                extractor = ClassMemberVariableExtractor(self, line)
        if extractor is not None:
            more = extractor.process_line("")
            assert (not more), "expected the extractor to complete"
            extractor.save(build)
