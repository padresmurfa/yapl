...
def main(parsed_args):
    ...
    types_path = Path(parsed_args.build) / "types"
    types_path.mkdir(parents=True, exist_ok=True)
    if parsed_args.force:
        shutil.rmtree(types_path, ignore_errors=False, onerror=None)
        types_path.mkdir(parents=True, exist_ok=True)
...
class Extractor:
    ...
    def get_filename(self, build):
        return str(Path(build) / "functions" / self._metadata["source"]["revision"] / self._metadata["identifier"])
...
class ModuleFunctionExtractor(Extractor):

    _expression = re.compile(r"^(private\s+){0,1}\s*("
                             r"((function)|(generator)|(closure))"
                             r"(\-((function)|(generator)|(closure)))*"
                             r")\s+(.+)")
    _identifier = re.compile(r"\s*([^\(]+)(.*)")

    def __init__(self, parent, line):
        super().__init__(parent._metadata, "functions")
        self._parent = parent
        m = ModuleFunctionExtractor._expression.match(line)
        ...
        m = ModuleFunctionExtractor._identifier.match(remainder)
        _name = m.group(1)
        remainder = m.group(len(m.groups()))
        description = describe("function", remainder)
        ...
        if "returns" in description:
            self._metadata["function"]["returns"] = description["returns"]
        elif "yields" in description:
            self._metadata["function"]["yields"] = description["yields"]
    ...
    def get_filename(self, build):
        _path = self.get_path(build)
        return str(Path(_path) / self._metadata["function"]["name"])
