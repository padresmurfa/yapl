...
def main(parsed_args):
    ...
    class_methods_path = Path(parsed_args.build) / "class_methods"
    class_methods_path.mkdir(parents=True, exist_ok=True)
    if parsed_args.force:
        shutil.rmtree(class_methods_path, ignore_errors=False, onerror=None)
        class_methods_path.mkdir(parents=True, exist_ok=True)
...
class Extractor:
    ...
    def get_filename(self, build):
        return str(Path(build) / "class_methods" / self._metadata["source"]["revision"] / self._metadata["identifier"])
...
class ClassMethodExtractor(Extractor):
    ...
    def get_filename(self, build):
        _path = self.get_path(build)
        return str(Path(_path) / self._metadata["method"]["name"])

    def get_path(self, build):
        _path = str(Path(build) / "class_methods" / self._metadata["source"]["revision"] / self._metadata["class"]["name"])
        return str(_path)
